...
class QuizApp {
    ...
    startQuiz(reuseQuestions = false) {
        if (!reuseQuestions) {
            // Get the ID range input
            const idRangeInput = document.getElementById("id-range").value.trim();
            let filteredQuestions = [...this.quizData.questions];
            
            // Parse and filter by ID range if specified
            if (idRangeInput) {
                const rangeMatch = idRangeInput.match(/^(\d+)-(\d+)$/);
                if (rangeMatch) {
                    const startId = parseInt(rangeMatch[1], 10);
                    const endId = parseInt(rangeMatch[2], 10);
                    
                    if (startId <= endId) {
                        filteredQuestions = this.quizData.questions.filter(q => 
                            q.id >= startId && q.id <= endId
                        );
                        
                        if (filteredQuestions.length === 0) {
                            alert(`No questions found in the ID range ${startId}-${endId}`);
                            return;
                        }
                    } else {
                        alert("Invalid range: start ID must be less than or equal to end ID");
                        return;
                    }
                } else {
                    alert("Invalid format. Please use format: start-end (e.g., 1-100)");
                    return;
                }
            }
            
            // Get question count and shuffle
            const count = parseInt(document.getElementById("question-count").value, 10) || filteredQuestions.length;
            
            // Adjust count if it exceeds available filtered questions
            const actualCount = Math.min(count, filteredQuestions.length);
            
            if (actualCount < count) {
                alert(`Only ${filteredQuestions.length} questions available in the specified range. Showing all ${actualCount} questions.`);
            }
            
            this.questions = this.shuffle(filteredQuestions).slice(0, actualCount);
        }
